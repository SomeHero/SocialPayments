...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        logger.Log(LogLevel.Info, String.Format("Workflow Started"));
        ...
        if (InputStreamContents.StartsWith("{") == true)
        {
            ...
            {
                var paymentId = MyObjectDictionary["Message"].ToString();
                    
                logger.Log(LogLevel.Info, String.Format("Posted to Workflow {0}", paymentId.ToString()));
                ...
                if (IsSignatureValid == true)
                {
                    logger.Log(LogLevel.Info, String.Format("Processing Payment Workflow {0}", paymentId.ToString()));
                
                    var paymentWorkflow = new SocialPayments.Workflows.Payments.PaymentWorkflow();
                    paymentWorkflow.ProcessPayment(paymentId);
                }
                else
                {
                    LogMessage += "The Signature is not valid.  No action was preformed." + Environment.NewLine;
