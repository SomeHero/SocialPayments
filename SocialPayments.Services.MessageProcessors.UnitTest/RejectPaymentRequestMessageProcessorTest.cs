...
{
    [TestClass]
    public class RejectPaymentRequestMessageProcessorTest
    {
        private IDbContext _ctx;
        private Logger _logger;
        private RejectPaymentRequestMessageProcessor _rejectPaymentRequestMessageProcessor;
        ...
        public RejectPaymentRequestMessageProcessorTest()
        {
            ...
            _rejectPaymentRequestMessageProcessor = new RejectPaymentRequestMessageProcessor(_ctx, _logger);
        }
        [TestMethod]
        public void WhenRequestIsRejectedThenNoTransactionsAreCreatedAndMessageStatusIsReject()
        {
            ...
            //Process the payment request
            _rejectPaymentRequestMessageProcessor.Process(message);

            var transactionBatch = _transactionBatchServices.GetOpenBatch();

            //Assert there are two transaction in batch
            Assert.AreEqual(0, transactionBatch.Transactions.Count);

            //Assert that the Status of the Request is Pending
            Assert.AreEqual(Domain.MessageStatus.RequestRejected, message.MessageStatus);
        }
    }
}
