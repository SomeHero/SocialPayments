...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (InputStreamContents.StartsWith("{") == true)
        {
            ...
            {
                var paymentRequestId = MyObjectDictionary["Message"].ToString();
                    
                logger.Log(LogLevel.Info, String.Format("Posted to Workflow {0}", paymentRequestId.ToString()));
                ...
                if (IsSignatureValid == true)
                {
                    var paymentWorkflow = new SocialPayments.Workflows.PaymentRequests.PaymentRequestWorkflow();
                    paymentWorkflow.ProcessPayment(paymentRequestId);
                }
                else
                {
                    LogMessage += "The Signature is not valid.  No action was preformed." + Environment.NewLine;
